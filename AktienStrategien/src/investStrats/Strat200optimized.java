...
public class Strat200optimized {
	...
	public static void firstEntry(String ticker, BigDecimal start) {
		try {
			PreparedStatement pstatement = Database.connection.prepareStatement("replace into " + ticker
					+ "200stratOptimized (day, ticker, flag, count, konto) values (?,?,?,?,?)");
	...
	public static void insertIntoTable(LocalDate date, String ticker, boolean flag, int count, BigDecimal konto) {
		try {
			PreparedStatement pstatement = Database.connection.prepareStatement(
					"replace into " + ticker + "200stratOptimized(day, ticker, flag, count, konto) values (?,?,?,?,?)");
	...
	public static void getInsertData(LocalDate date, String ticker) {
		...
		try {
			...
			Database.dropTable(ticker + "200stratOptimized");
		}
	}
	...
	public static boolean boughtOrSold(String ticker) {
		try {
			PreparedStatement pstatement = Database.connection
					.prepareStatement("select flag from " + ticker + "200stratOptimized order by day desc limit 1");
	...
	public static void sellOrBuy(LocalDate date, String ticker) {
		...
		if (boughtOrSold(ticker)) {
			if (compareBigger(close, get200avg)) {
			...
			if (compareSmaller(close, get200avg)) {
	...
	public static BigDecimal moneyBefore(LocalDate date, String ticker) {
		...
		try {
			pstatement = Database.connection
					.prepareStatement("select konto from " + ticker + "200stratOptimized order by day desc limit 1");
	...
	public static BigDecimal getCount(LocalDate date, String ticker) {
		...
		try {
			pstatement = Database.connection
					.prepareStatement("select count from " + ticker + "200stratOptimized order by day desc limit 1");
			ResultSet result = pstatement.executeQuery();
			result.next();
			return result.getBigDecimal(1);
		} catch (SQLException e) {
	...
	public static boolean compareBigger(BigDecimal close, BigDecimal avg200) {
	...
	public static boolean compareSmaller(BigDecimal close, BigDecimal avg200) {
